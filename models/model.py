...
class GRU_CNN_Attention(nn.Module):
    ...
    def forward(self, x, text_masks, post_masks, use_gpu=False):
        ...
        # ✅ 第二层：融合 V_dynamic 和 V_common → V_long
        ...
        # ✅ 第三层：门控融合 V_long 和 V_short → V_final
        V_final = self.gated_final_fusion(V_long, V_short)  # (B, 2H)
        V_final = self.final_drop(V_final)  # 应用最终特征Dropout
